...
void Radio_init(const void* context)
{
		...
		const sx126x_pkt_params_lora_t lora_pkt_params = {
		    ...
		    .header_type = SX126X_LORA_PKT_EXPLICIT,  // 0x00
		    .pld_len_in_bytes = 16,  // Match buffer size
		    .crc_is_on = false,
...
*/
	  ...
	  printf("Packet config: header_type=%d, pld_len=%d\n\r",
	         lora_pkt_params.header_type, lora_pkt_params.pld_len_in_bytes);
